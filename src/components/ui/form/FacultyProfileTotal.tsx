...
function isUniform<T>(input: T[]): boolean {
    const base = input[0];
...
export function RowTotal({
    ...
    return (
        <form.Subscribe
            selector={(state) =>
                ((state.values as AnyRow)[fieldName] as AnyRow[])?.[index]
            }
        >
...
export function ColumnTotal({
    ...
    const form = useFormContext()
    return (
        <form.Subscribe selector={(state) => (state.values as AnyRow)[fieldName] as AnyRow[]}>
            {(rows) => {
                const total = sumBy(rows ?? [], (row) => row ? Number(row[targetCol] || 0) : 0)
...
export function FacultyRowTotal({ index }: { index: number }) {
    return <RowTotal index={index} fieldName="faculty" calculate={calculateTotalFacultyRow} />
}
